...
function restartFromCheckpoint(countAttempt = true) {
  ...
  player.x = spawn.x;
  player.y = spawn.y;
  player.vx = spawn.mode === "plane" ? 300 : 260;
  player.vy = 0;
...
function targetSpeed() {
  ...
  let speed = player.mini ? 338 : 305;
  if (section.id === "spikes" || section.id === "cube-return") speed = 322;
  if (section.id === "mini" || section.id === "mix") speed = 342;
...
function updateCube(dt) {
  ...
  const target = player.yellowActive ? 382 : targetSpeed();
  player.vx += (target - player.vx) * Math.min(1, dt * 7.2);
  player.vx = Math.max(230, Math.min(player.yellowActive ? 410 : 390, player.vx));
...
function updatePlane(dt) {
  player.vx += (318 - player.vx) * Math.min(1, dt * 7);
